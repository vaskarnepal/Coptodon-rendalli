function [prdData, info] = predict_Coptodon_rendalli(par, data, auxData)
  ...
  filterChecks =  k * v_Hp >= f_tL^3 || ...                                         % constraint required for reaching puberty with f_tL
                 ...
                 f_LW0 > 1.5;                                                       % Note: Consumption in wild can be twice that in captivity (which is assumed to be as high as 1, here);
...
%   TC_T_JO    = tempcorr(C2K(T_JO(:,1)), T_ref, pars_T);
  ...
  %% Length-weight
  prdData.LWw  = (LWw(:,1) * del_M).^3 * (1 + f_LW0 * w);                  % g, expected wet weight at time
  
  %% time since birth-length 
  prdData.tL  = get_Lw(f_tL, TC_tp, tL,  par, cPar);                       % Juveniles and adults; WeylHech1988
  prdData.tL0 = get_Lw(f_tL, TC_tL0, tL0,  par, cPar);                     % Larvae; MoriMats2007; assume feeding is not at max (because wrong food, stress, etc.)
  
  %% Wet-weight v fecundity
  pars_R = [kap; kap_R; g; k_J; k_M; L_T; v; U_Hb; U_Hp];                  % compose parameter vector at T
  Length = (WwR(:,1) / (1 + f_tL1 * w)).^(1/3);                            % Structural length at T and f_tL1
  prdData.WwR = 365 * TC_WwR * reprod_rate(Length, f_tL1, pars_R);         % #, fecundity per year
...
%   prdData.T_JO = - J_Min(:,3) .* TC_T_JO * 32e3/ Ww_0;                     % mg O2/d.g, spec O2 consumption
  ...
  prdData.tdW_T20   = get_tdW(f, TC_tdW_T20, tdW_T20, 6.66, par, cPar);         % 20.5 C
%   prdData.tdW_T20   = L_T20.^3 * (1 + f * w);                              % g, expected wet weight at time
  prdData.tdW_T24   = get_tdW(f, TC_tdW_T24, tdW_T24, 6.69, par, cPar);         % 24 C
%   prdData.tdW_T24   = L_T24.^3 * (1 + f * w);                              % g, expected wet weight at time
  prdData.tdW_T28   = get_tdW(f, TC_tdW_T28, tdW_T28, 6.6, par, cPar);         % 28 C
%   prdData.tdW_T28   = L_T28.^3 * (1 + f * w);                              % g, expected wet weight at time
  prdData.tdW_T32   = get_tdW(f, TC_tdW_T32, tdW_T32, 6.62, par, cPar);         % 32 C
%   prdData.tdW_T32   = L_T32.^3 * (1 + f * w);                              % g, expected wet weight at time
...
% VB growth model when initial age/size is NOT the age/size at birth
 function EW = get_tdW(f, TC, dataset, init_size, p, c)              % Function get_tdW that gives output EW
L_0   = (init_size / (1 + f * c.w)) .^ (1/3);                                        % cm, structural length
